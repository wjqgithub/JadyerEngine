...
public final class MinaUtil {
	...
	/**
	 * 向支付处理系统发送TCP消息
	 * @see 该方法默认采用"GB18030"字符集编解码数据
	 * @see 当通信发生异常时,如Fail to get session....返回<code>"MINA_SERVER_ERROR"</code>字符串
	 */
	public static String sendTCPMessage(String message){
		String adderss = ConfigUtil.INSTANCE.getProperty("client.tcp.payCore");
		return sendTCPMessage(message, adderss.split(":")[0], Integer.parseInt(adderss.split(":")[1]), ConfigUtil.INSTANCE.getProperty("sys.charset.tcp"));
	}
	 ...
	 * @see 当通信发生异常时,如Fail to get session....返回<code>"MINA_SERVER_ERROR"</code>字符串
	 */
	public static String sendTCPMessageToWoPortal(String message){
		String adderss = ConfigUtil.INSTANCE.getProperty("client.tcp.woPortal");
		return sendTCPMessage(message, adderss.split(":")[0], Integer.parseInt(adderss.split(":")[1]), ConfigUtil.INSTANCE.getProperty("sys.charset.tcp"));
	}
	...
	public static String sendTCPMessage(String message, String ipAddress, int port, String charset){
		...
		connector.setConnectTimeoutMillis(ConfigUtil.INSTANCE.getPropertyForInt("sys.timeout.tcp.connect"));
		...
		try{
			...
			if(readFuture.awaitUninterruptibly(ConfigUtil.INSTANCE.getPropertyForInt("sys.timeout.tcp.bothIdle"), TimeUnit.SECONDS)){
