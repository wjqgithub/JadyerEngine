 ...
 * -----------------------------------------------------------------------------------------------------------
 * 邮件发送
 * 0)发邮件的话,由于配置日志级别是ERROR,所以执行LogUtil.getLogger().error("....")就发出去了
 *   在本例中默认已在log4j.properties文件中注释了邮件发送的配置
 * 1)log4j-1.2.14之后才支持SMTP认证,否则会报告异常,此工具类用的是最新的log4j-1.2.17.jar
 ...
 * 6)Unix和Linux一般都会默认开启SMTP服务的,我们可以在Linux上执行该命令查看[telnet 127.0.0.1 25]
...
public final class LogUtil {
	 ...
	 * 我博客里对此有记载http://blog.csdn.net/jadyer/article/details/7338071
	 * Struts2就是这么做的,详见com.opensymphony.xwork2.ActionContex第43和166行源码
	 * </p>
	 */
